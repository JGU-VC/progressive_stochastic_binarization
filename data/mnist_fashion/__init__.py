...
class DataSampler(BaseDataSampler):
    ...
    def __init__(self, data_dir):
        self.data_dir = os.path.join(data_dir,"mnist_fashion")
    ...
    def training(self):
        if not hasattr(DataSampler,"trainvaldata"):
            DataSampler.trainvaldata = dataset(str(self.data_dir), 'train-images-idx3-ubyte', 'train-labels-idx1-ubyte')
        return DataSampler.trainvaldata.take(numexamples)

    def validation(self):
        if not hasattr(DataSampler,"trainvaldata"):
            DataSampler.trainvaldata = dataset(str(self.data_dir), 'train-images-idx3-ubyte', 'train-labels-idx1-ubyte')
        return DataSampler.trainvaldata.skip(numexamples) # 5000 left
